...
export default function SchedulePage() {
  ...
  useEffect(() => {
    async function fetchNews() {
      setLoading(true);
      try {
        const url = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/news?is_published=eq.true&category=eq.schedule&select=id,slug,title,excerpt,author,published_at&order=published_at.desc`;
        ...
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/80 mt-2">행사 및 활동 일정을 안내합니다.</p>
            </div>
          </div>
        </div>
      </section>
          ...
          {filteredSchedule.length === 0 ? (
            ...
            <>
              <div className="mb-6 text-[var(--gray-600)]">
                총 <span className="font-semibold text-[var(--primary)]">{filteredSchedule.length}</span>개의 일정
              </div>
              <div className="space-y-4">
                {filteredSchedule.map((news) => (
