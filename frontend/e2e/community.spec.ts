...
test.describe('커뮤니티 페이지', () => {
  ...
  test('지역 커뮤니티 섹션이 표시된다', async ({ page }) => {
    ...
    await expect(regionSection.first()).toBeVisible({ timeout: 15000 });
  });

  test('상임위원회 커뮤니티 섹션이 표시된다', async ({ page }) => {
    // Look for committee or 위원회 text
    const committeeSection = page.locator('text=/상임위|위원회/');
    await expect(committeeSection.first()).toBeVisible({ timeout: 15000 });
  });
});
